...
async function train(model, data) {
    ...
    const container = {name: 'Model Training', styles: {height: '640px', outerHeight:'200px'}};// YOUR CODE HERE   
    ...
    const [trainXs, trainYs] = tf.tidy(() => { // YOUR CODE HERE
        ...
        return [
            dt.xs.reshape([TRAIN_DATA_SIZE, 28, 28, 1]),
            dt.labels
        ]
    })
    ...
    const [testXs, testYs] = tf.tidy(() => { // YOUR CODE HERE
        const dt = data.nextTrainBatch(TEST_DATA_SIZE);
        return [
            dt.xs.reshape([TEST_DATA_SIZE, 28, 28, 1]),
            dt.labels
        ]
    })
